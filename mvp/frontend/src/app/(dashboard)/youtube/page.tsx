...
import { autoChapter } from '@/features/transcription/api';
import type { AutoChapterResponse } from '@/features/transcription/types';
import { analyzeYTVideo, captureYTStream, checkYTStreamStatus, getYouTubeMetadata, smartTranscribeYT, transcribeYTPlaylist } from '@/features/youtube-transcription/api';
import type { PlaylistResult, StreamCaptureResult, StreamStatusResult, TranscriptResult, VideoAnalyzeResult, YouTubeMetadata } from '@/features/youtube-transcription/types';
...
export default function YouTubePage() {
  ...
  const smartMutation = useMutation<TranscriptResult, Error, void>({
    mutationFn: () => smartTranscribeYT(url, language),
  });
  const metadataMutation = useMutation<YouTubeMetadata, Error, void>({
    mutationFn: () => getYouTubeMetadata(url),
  });
  const playlistMutation = useMutation<PlaylistResult, Error, void>({
    mutationFn: () => transcribeYTPlaylist(url, language),
  });
  const chapterMutation = useMutation<AutoChapterResponse, Error, void>({
    mutationFn: () => autoChapter(url),
  });
  const streamStatusMutation = useMutation<StreamStatusResult, Error, void>({
    mutationFn: () => checkYTStreamStatus(url),
  });
  const captureMutation = useMutation<StreamCaptureResult, Error, void>({
    mutationFn: () => captureYTStream(url, 300),
  });
  const videoAnalyzeMutation = useMutation<VideoAnalyzeResult, Error, void>({
    mutationFn: () => analyzeYTVideo(url),
  });
  ...
  return (
    <div className="p-5 space-y-5 animate-enter">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-[var(--text-high)]">YouTube Studio</h1>
          <p className="text-xs text-[var(--text-mid)]">YouTube channel analytics and management</p>
        </div>
      </div>
      ...
      {/* Metadata Result */}
      {metadataMutation.data && <div><MetadataCard data={metadataMutation.data} /></div>}
      ...
      {/* Errors */}
      {smartMutation.isError && <Alert variant="destructive"><AlertDescription>{smartMutation.error.message}</AlertDescription></Alert>}
      {metadataMutation.isError && <Alert variant="destructive"><AlertDescription>{metadataMutation.error.message}</AlertDescription></Alert>}
      {playlistMutation.isError && <Alert variant="destructive"><AlertDescription>{playlistMutation.error.message}</AlertDescription></Alert>}
      {chapterMutation.isError && <Alert variant="destructive"><AlertDescription>{chapterMutation.error.message}</AlertDescription></Alert>}
      {captureMutation.isError && <Alert variant="destructive"><AlertDescription>{captureMutation.error.message}</AlertDescription></Alert>}
      {videoAnalyzeMutation.isError && <Alert variant="destructive"><AlertDescription>{videoAnalyzeMutation.error.message}</AlertDescription></Alert>}
    </div>
  );
}
